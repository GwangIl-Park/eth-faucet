...
export const FaucetContent = () => {
  ...
  const onClickBtn = () => {
    setIsLoading(true);
    instance.post('/faucet/request', {
      wallet_address: address
    })
      .then((response) => {
        setEthAmount((Number(response.data.ethBalance)/(10**18)).toFixed(6))
        setTokenAmount((Number(response.data.tokenBalance)/(10**18)).toFixed(6))
        setShowText(true)
        setIsLoading(false);
  ...
  const onClickETH = () => {
    setIsLoading(true);
    instance.post('/faucetETH/request', {
      wallet_address: address
    })
      .then((response) => {
  ...
  const onClickToken = () => {
    setIsLoading(true);
    instance.post('/faucetToken/request', {
      wallet_address: address
    })
      .then((response) => {
        setEthAmount(response.data.ethBalance)
  ...
  return (
    <div className="faucet-container">
      ...
      <Input name="Address" type="string" onChange={onChange} placeholder="Type ETH Wallet Address Here" value={address} />
      {
        isLoading ? (<p>신청 중입니다..</p>) : <Button onClick={onClickBtn}>Request</Button>
      }
      <br/>
      {showText ? <div className='balance-container'><span>ETH : </span><span>{ethAmount}</span><span>  </span><span>Token : </span><span>{tokenAmount}</span></div> : null}
    </div>
  );
};