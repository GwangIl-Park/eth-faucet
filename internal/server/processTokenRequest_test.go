...
type TokenRequestTestSuite struct {
	suite.Suite
	server *server.Server
}

func (ts *TokenRequestTestSuite) SetupSuite() {
...
func (ts *TokenRequestTestSuite) SetupTest() {
}

func (ts *TokenRequestTestSuite) BeforeTest(suiteName, testName string) {
	fmt.Printf("Test :: suiteName: %s, testName: %s\n", suiteName, testName)
}

func (ts *TokenRequestTestSuite) TestRequestToken() {
	...
	_, err := client.RequestToken(context.Background(), &faucetToken.FaucetTokenRequest{WalletAddress: "0x7d391b793e208f515a40c08dc87a2af1e53ffd9A"})

	assert.NoError(ts.T(), err, "RequestToken should not return error")

	ethAfter, tokenAfter, _ := ts.server.GetEthTokenBalance(&address)

	assert.Equal(ts.T(), ethBefore, ethAfter, "RequestToken ethBalance should equal")
	assert.Equal(ts.T(), tokenBefore+1, tokenAfter, "RequestToken tokenBalance should equal +1")

	_, err = client.RequestToken(context.Background(), &faucetToken.FaucetTokenRequest{WalletAddress: "0x7d391b793e208f515a40c08dc87a2af1e53ffd9A"})
	assert.Error(ts.T(), err, "RequestToken in limit should return error")
}

func (ts *TokenRequestTestSuite) AfterTest(suiteName, testName string) {
	fmt.Printf("Test :: suiteName: %s, testName: %s End\n", suiteName, testName)
}

// suite에서 각 테스트 실행후에 실행된다
func (ts *TokenRequestTestSuite) TearDownTest() {
}

// suite에서 모든 테스트가 실행된 후에 실행된다
func (ts *TokenRequestTestSuite) TearDownSuite() {
	...
	fmt.Printf("processTokenRequest.go Test End\n")
}

func TestTokenRequest(t *testing.T) {
	suite.Run(t, new(TokenRequestTestSuite))
}
